...
class Asistencia {
    ...
    public function marcar($codigo_estudiante, $codigo_tutor, $tipo) {
        if (!in_array($tipo, [self::ASISTENCIA_ASISTIO, self::ASISTENCIA_INASISTENCIA, self::ASISTENCIA_JUSTIFICADO])) {
        ...
        try {
            ...
            $resultado = $existe
                ? $this->actualizarAsistencia($codigo_estudiante, $codigo_tutor, $tipo, $fecha)
                : $this->crearAsistencia($codigo_estudiante, $codigo_tutor, $tipo, $fecha);
    ...
    private function verificarAsistenciaExistente($codigo_estudiante, $fecha) {
        $query = "SELECT 1 FROM {$this->table} WHERE fecha = ? AND codigo_estudiante = ? LIMIT 1";
    ...
    public static function getDescripcionTipo($tipo) {
        switch ($tipo) {
            case self::ASISTENCIA_ASISTIO:
                return 'Asistió';
            case self::ASISTENCIA_INASISTENCIA:
                return 'Inasistencia';
            case self::ASISTENCIA_JUSTIFICADO:
                return 'Justificado';
            default:
                return 'Desconocido';
        }
